...
function DataPresentation(row, property) {
    ...
    } else if (property == 'employee_id') {
        return <Link to={routes.showEmployee.replace(':id', row[property])}>{row['employee'].name}</Link>

    } else if (property == 'reporter_id') {
        return <Link to={routes.showEmployee.replace(':id', row[property])}>{row['reporter'].name}</Link>
...
export default function TrialPeriodAssessmentTable(props) {
    ...
    const [dataShow, setdataShow] = React.useState({
        ...
        employee_id: { visiblity: true, label: 'الموظف', presentation: (row) => DataPresentation(row, 'employee_id') },
        reporter_id: { visiblity: true, label: 'معد التقرير', presentation: (row) => DataPresentation(row, 'reporter_id') },
